...
class TagParse:
    ...
    def read_data(self, base_data_root, condition_data_root, pickle_sub_path):
        ...
        condition_pickle_path = osp.join(condition_data_root, pickle_sub_path.replace('.pickle', 'ego_path.pickle'))
    ...
    def save_data(self, save_data_root, pickle_sub_path):
        os.makedirs(osp.join(save_data_root, '/'.join(pickle_sub_path.split('/')[:-1])), exist_ok=True)
...
if __name__ == '__main__':
    ...
    tag_parse.process(base_data_root, condition_data_root, save_root, json_list[0])
